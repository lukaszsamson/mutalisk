...
**v1.28 outcome (2026-06-02):** the first maturation-arc release shipped, trust over growth as intended. M99 confirmed and regression-locked the three trust-critical correctness fixes from the bug-review pass — timeout now counts as a detection and the terminal score provably equals the Stryker-viewer-derived score on the same report; UTF-8 byte-offsets are codepoint-correct end-to-end (EnvWalker span → FallbackPatch on non-ASCII source); the timeout hot path tree-kills (a test spawns a wrapper that forks a child and asserts both are reaped). M100 added `.mutalisk.exs` config loading with documented precedence (file < `config :mut` < CLI), threaded `exclude` through the options struct, and shipped `@mutalisk_ignore true` module-level source ignores (nesting-aware fully-qualified detection; recorded as `:mutalisk_ignore` skips). M101 added opt-in `Mut.Reporter.Html` (self-contained survivor report) and `Mut.Reporter.GitHubActions` (`::warning` PR annotations), both consuming the Stryker JSON map the tool already builds, neither changing the score or the default reporters (`@default_reporters` split from `@known_reporters`). M102 rewrote the README user-facing (interpret the score, handle surviving mutants, config, reporters) and added `docs/MUTATORS.md` (the 13-default-on / 16-opt-in catalogue); the dead-code prune was grep-confirmed clean (the v1.15 persistent-worker removal left only the intentional `--worker-type` deprecation shim; no orphaned modules). M103 measured four strong-semantic FunctionReplace pair candidates (Enum.sort↔reverse, take_while↔drop_while, String.upcase↔downcase, Map.put↔put_new): all invalid-safe (0%) but with **no kill-signal** on the available targets (jason added 2 candidates, both equivalent; bandit added 0) — so, consistent with the M60/M88/M93/M95 data-gated discipline, they were **held out of the default-on allowlist** rather than shipped on thin data, since a mostly-equivalent pair in a default-on surface depresses users' scores. Throughout: default plan + score byte-identical (golden gates green), Elixir floor `>= 1.19.0`, release constraint held (local on master, no push/tag). The product is now meaningfully closer to "a stranger can run this and trust the number."

### v1.29 (5 milestones — incremental cross-run history + dead-code cleanup)

v1.29 reverses the standing incremental-history hold. Across releases it was held as "CI-only value, reopen only on an explicit CI-adoption decision" — the user has now made that decision. This is the long-deferred v2 perf bet, and v1.29 implements the design the HLD already specifies (see `### Incremental History`): history key `stable_mutant_id + source_digest + selected_tests_digest + config_digest`, conservative reuse, "incorrect reuse is worse than a slow run." Alongside it, a dead-code / failed-experiment cleanup track using the user's `ex_crap` CRAP-analysis tool (or coverage-report-driven dead-code detection as the fallback).

The foundation already exists: `bench/cross_run.exs` (the M86 redirect) proved the Stryker JSON report carries enough per-mutant state (stable_id + status + duration + engine + mutator) to compute meaningful cross-run deltas with **no engine change**. v1.29 turns that read-only prelude into a persistent, reuse-driving feature.

**The correctness bar is the whole game.** Mutation testing's value is a trusted number; a stale-reuse bug produces a wrong number that looks right — strictly worse than a slow run. History is therefore **opt-in** (`--incremental` / config), conservative by construction (reuse only on exact digest match), and validated against full-run ground truth before it can be trusted. Defaults do not change: a normal `mix mut` with no `--incremental` is byte-identical to v1.28. Engine execution model is untouched (no reopening the port-driven sandbox — the trust anchor). Elixir floor stays `>= 1.19.0`; release constraint holds.

Two tracks. History: M104 (spike) → M105 (store + digests) → M106 (reuse + flag) → M107 (validation). Cleanup: M108 (independent; can interleave).

**M104 — Incremental history design spike.** Spike-first, because the correctness bar is high and several design choices have real consequences. Decide and document in `docs/spikes/M104_incremental_history.md`: (1) **digest granularity** — `source_digest` at function-level vs span-level vs file-level (function-level is the likely sweet spot: a mutant's verdict invalidates when its enclosing function's source changes, not when any byte in the file changes); (2) **store format + location** — JSON/ETS-dump under `_build/mut_history/` or a configured path; not in the user's source tree; (3) **the reuse decision table** made concrete per status (killed/survived/timeout/error/invalid) against the four key components; (4) **selected_tests_digest** derivation from the coverage/static selector output; (5) **config_digest** inputs (which config changes must invalidate — enabled targets, timeout, selection mode); (6) **invalidation + GC** (stale entries pruned; store can't grow unbounded). Acceptance: design doc committed; reuse table concrete per status; digest granularity decided with rationale; no production code beyond a throwaway proof.

**M105 — Persistent verdict store + digest computation.** Implement M104's design: `Mut.History.Store` (read/write the per-mutant verdict store keyed by stable_id, carrying status + the four digests + killing-test identity for killed mutants) and `Mut.History.Digest` (compute `source_digest` at the chosen granularity, `selected_tests_digest`, `config_digest`). Writes to the isolated history path, never the user tree. No reuse yet — this milestone only *records* history at end of run and proves round-trip read. Acceptance: a run writes a complete, valid store; re-reading reproduces every verdict + digest; store location is isolated + configurable; default run (no `--incremental`) writes history but changes nothing observable; `bin/verify` green.

**M106 — Reuse policy + `--incremental` flag.** The feature. `Mut.History.Reuse` applies the M104 decision table: for each planned mutant, if a stored verdict exists with matching digests per its status's reuse rule, skip execution and adopt the stored verdict; otherwise execute normally. `--incremental` (+ config) opts in; `--since <ref>` optionally diff-scopes to mutants in changed files. Conservative: any digest mismatch, missing entry, or ambiguous case → execute. Reused vs executed counts surfaced in metrics + terminal + Stryker JSON. Acceptance: a no-change re-run reuses ~all verdicts (executes ~none); changing one function re-executes only that function's mutants; killed-mutant reuse requires the killing test unchanged; survivor reuse requires all selected tests unchanged; `--incremental` absent ⇒ v1.28 behavior byte-identical; `bin/verify` green.

**M107 — Incremental validation on the OSS matrix.** The trust gate. Across the matrix: (1) **ground-truth correctness** — a full run and an incremental run on the same unchanged tree produce identical per-mutant verdicts and identical score (reuse must never change the answer); (2) **diff-scoped correctness** — mutate a file, run incremental, confirm only affected mutants re-execute and the result matches a full run; (3) **wall-clock win** — document the speedup on a small-diff re-run (the CI scenario); (4) **invalidation correctness** — changing config/timeout/selection invalidates the right entries. Decision doc `docs/decisions/M107_incremental_history.md` with the correctness evidence + the CI-usage guide. Acceptance: zero verdict divergence between full and incremental on unchanged trees across the matrix; diff-scoped runs match full-run ground truth; speedup documented; BENCHMARKS v1.29 section.

**M108 — Dead-code + failed-experiment cleanup (`ex_crap` / coverage-driven).** Use the user's `ex_crap` CRAP-analysis tool (`../ex_crap`, if available — else coverage-report-driven dead-code detection) to find high-CRAP and unreachable code, then prune dead code and failed-experiment remnants. v1.28/M102's grep-prune found the persistent-worker removal clean except the intentional `--worker-type` shim; M108 goes deeper — coverage/CRAP analysis surfaces code that grep can't (unreachable branches, never-called private helpers, spike/redirect leftovers like abandoned-optimization paths). Every removal must be provably dead (covered by the analysis + a grep for call sites) and gated by `bin/verify` staying green. Acceptance: high-CRAP/dead-code report produced; provably-dead code removed (each removal justified); no behavior change (golden gates + full suite green); `bin/verify` green.

**Explicitly NOT in v1.29:**
- Default `--incremental` (stays opt-in until real CI validation beyond M107's matrix proves it; flipping a reuse default is a future decision).
- Engine optimization / reopening the sandbox model (history reuses verdicts; it does not change how mutants execute).
- New mutators / new mutation surface (catalogue closed; M103 was the last allowlisted attempt).
- Function-call deletion / return-value replacement (high-FP).
- Pushing / tagging / Hex publish (release constraint holds).

**v1.29 OUTCOME (SHIPPED 2026-06-03).** All five milestones landed
(`b0ac349..9cbe7bf`, unpushed). Incremental cross-run history ships **opt-in**
(`--incremental`), correctness-first. The trust gate (M107) proved **zero
verdict divergence** between full and incremental runs on both demo_app
(CI-locked) and decimal (real OSS: 80.0 == 80.0, 89/90 reused), diff-scoped
reuse re-executes only the edited function and matches full-run ground truth,
and the wall-clock win scales with mutant count (decimal 156s → 16s ≈ 9.75×).
Design refinement: `source_digest` is **function-level** (tightest *safe* unit —
span-level is unsound, file-level kills the win); the killing test is recorded
only as an ExUnit identifier, so killed reuse gates on `selected_tests_digest`
like survived. Defaults unchanged (no-`--incremental` is v1.28-byte-identical).
