defmodule Mut.Mutator.UnaryNot do
  @moduledoc "Removes unary not operators."
  ...
  @impl true
  def name, do: "UnaryNot"

  @impl true
  def description, do: "Remove unary not operators."
  ...
  defp build_mutations({op, _meta, [arg]} = node) do
    [
      %Mutation{
        original_ast: node,
        mutated_ast: arg,
        description: "remove unary #{op}",
        mutation_kind: @kind,
        guard_safe?: true,
        metadata: %{operator: op, replacement: nil}
      }
    ]
  end
end
